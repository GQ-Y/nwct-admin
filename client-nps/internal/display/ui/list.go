...
// Render 渲染列表
func (l *List) Render(g Graphics) error {
...
// renderItem 渲染单个列表项
func (l *List) renderItem(g *display.Graphics, item *ListItem, y int, selected bool) {
	// 背景
	...
	g.DrawTextTTF(item.Title, l.X+20, y+28, titleColor, 16, display.FontWeightMedium)
	...
	if item.Subtitle != "" {
		...
		g.DrawTextTTF(item.Subtitle, l.X+20, y+50, subtitleColor, 12, display.FontWeightRegular)
	}
